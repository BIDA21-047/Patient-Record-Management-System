...
def login(username, password):
    ...
    cursor.execute("SELECT * FROM Doctor WHERE username = ? AND password = ?", (username, hashed_password))
...
def homepage(id):
    ...
    subprocess.run(["python", "Homepage.py", id, "Dr. "])
...
def nextform():
    ...
    # Run another Python file using subprocess
    subprocess.run(["python", "DoctorRegistration.py"])
...
# Heading
heading = tk.Label(frame, text="Doctor Sign In", font=("Helvetica", 24, "bold"))
heading.pack(padx=5)
...
#  testing: next button
next_button = tk.Button(root, text="New Doctor, Registration", command=nextform)
...
heading.grid(row=1, column=0, padx=(0, 30), pady=5, sticky="e")
UserName_label.grid(row=8, column=0, padx=(20, 0), pady=5, sticky="w")
UserName_entry.grid(row=8, column=0, padx=(150, 0), pady=5, sticky="w")
Password_label.grid(row=9, column=0, padx=(20, 0), pady=5, sticky="w")
Password_entry.grid(row=9, column=0, padx=(150, 0), pady=5, sticky="w")
